...
import { TODAY, FROM_TODAY, LOGINUSER } from '../functions.js';
...
describe('DateField', () => {
  ...
  it('in演算子で複数の日付を指定できる', () => {
    ...
    expect(toString(expr)).toBe('契約日 in ("2024-01-01", "2024-02-01", "2024-03-01")');
  });
});
...
describe('フィールドの組み合わせ', () => {
  it('異なるフィールドタイプを組み合わせられる', () => {
    const 会社名Field = createStringField('会社名');
    const 売上高Field = createNumberField('売上高');
    const ステータスField = createDropdownField('ステータス', ['商談中', '受注'] as const);
